...
<head>
	...
	<script type="text/javascript">
						 ...
						 top.$.jBox(html, { title: "流程办理["+obj.data("tname") + "]",buttons:{"重新申请":"yes","放弃申请":"no","取消":"cancel"},submit: function (v, h, f) {
							 ...
							 if(v=="yes"|| v=="no") {
								complete(taskId,leaveId, [{
									...
									key: 'beginDate',
									value: top.$('#modifyApplyContent #beginDate').val(),
									type: 'D'
								}, {
									key: 'endDate',
		 ...
		 * @param {Object} taskId
		 */
		function complete(taskId,leaveId,variables) {
			// 转换JSON为字符串
	...
	<script type="text/template" id="auditTemplate">
		<table class="table table-striped ">
			...
			<tr>
				<td><font color="red">开始</font>时间：</td>
				<td>{{startTime}}</td>
			</tr>
			<tr>
				<td><font color="red">结束</font>时间：</td>
				<td>{{endTime}}</td>
			</tr>
			<tr>
				<td>请假事由：</td>
	...
	<script type="text/template" id="modifyApplyTemplate">
		<table class="table table-striped " id="modifyApplyContent">
			...
			<tr>
				<td>>开始时间：</td>
				<td>
					<input id="beginDate" name="beginDate" type="text" readonly="readonly" maxlength="20" class=" Wdate required"
						onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});" value="{{beginDate}}"/>
				</td>
			</tr>
			<tr>
				<td>结束时间：</td>
				<td>
					<input id="endDate" name="endDate" type="text" readonly="readonly" maxlength="20" class=" Wdate required"
						onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});" value="{{endDate}}"/>
				</td>
			</tr>
			<tr>
				<td>原因：</td>
...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="oa:leave:edit"><li><a href="${ctx}/oa/leave/form">申请</a></li></shiro:hasPermission>
	</ul>
