...
<head>
	...
	<script type="text/javascript">
		...
		function auditNo(){
			...
			var path = "${ctx}/yd/ydOvertime/audit?auditStatus=no";
			$('#inputForm').attr("action", path).submit();
		}

		function auditPass(){
			var path = "${ctx}/yd/ydOvertime/audit?auditStatus=pass";
			$('#inputForm').attr("action", path).submit();
		}
	</script>
...
<body>
	...
	<form:form id="inputForm" modelAttribute="ydOvertime"  method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<form:hidden path="erpNo" value="${ydOvertime.erpNo}"/>
		<form:hidden path="officeId" value="${ydOvertime.officeId}"/>
		<form:hidden path="auditLevel" value="${ydOvertime.auditLevel}"/>
		...
		<div class="form-actions">
			...
			<%--<shiro:hasPermission name="yd:ydOvertime:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>--%>
