...
public class WorkDayUtils {
	...
	public static void main(String[] args) {
		...
		System.out.println(app.isWorkDay(calendar));
		/*try {
	...
	public Boolean isWorkDay(Calendar calendar){
		String formatDate = DateUtils.formatDate(calendar.getTime(), null);
		Boolean flag = false;
		String weekDays[] = propertiesLoader.getValues("CMCC_WEEK_DAY", ",");
		String holtDay = propertiesLoader.getProperty("CMCC_HOLI_DAY");

		int dayOfWeek = calendar.get(Calendar.DAY_OF_WEEK) - 1;
		if (holtDay.contains(formatDate)){  //节假日需要配置
			return flag;
		}
		if (weekDays != null){
			for (String temp : weekDays){
				if (temp.equals(dayOfWeek + "") || formatDate.equals(temp)){
					flag = true;
				}
			}
		}
